...
Note that this design assume that all white/blank low variance patches will be systematically dropped even before any masking strategy.
...
1. **Target Encoder (Full Context):**
   The full, unmasked image crop is passed through the ViT Encoder. This produces a dense grid of target patch embeddings. We also compute a **Global Average Pool** of these embeddings.

2. **Context Encoder (Masked Input):**
   A large block of patches is dropped from the image. The *remaining* patches are passed through the exact same ViT Encoder (sharing weights with the Target Encoder).
