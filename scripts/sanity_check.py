...
def main():
    ...
    model = OMRDetector(backbone, num_classes=num_classes, num_shapes=5).to(device)
    ...
    if not img_dir.exists():
        print(f"Error: {img_dir} does not exist. Please ensure COCO128 is downloaded.")
        return
    ...
    image = torch.from_numpy(img_np).float().unsqueeze(0).to(device) / 255.0 # (1, 3, H, W)
    ...
    print("Sanity check complete. If the total loss dropped significantly (near 0), the architecture is learning!")
