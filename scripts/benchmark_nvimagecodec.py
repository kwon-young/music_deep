...
from nvimagecodec import Decoder
...
def benchmark(data_dir: Path, crop_size: int, iterations: int, image_name: str | None) -> None:
    ...
    if max_x < 0 or max_y < 0:
        print(f"Error: Crop size ({crop_size}) is larger than image dimensions ({w}x{h}).")
        return
    ...
    current_pipeline(src_img_path, coords[0][0], coords[0][1], crop_size, device)
    ...
    print(f"Current Pipeline (PIL PNG): {current_time:.4f}s ({iterations/current_time:.2f} it/s)")

    # --- 2. nvimagecodec Pipeline (JPEG) ---
    nvimagecodec_pipeline(jpg_img_path, coords[0][0], coords[0][1], crop_size, decoder)
    torch.cuda.synchronize()
    start_time = time.perf_counter()
    for x, y in coords:
    ...
    print(f"nvimagecodec   (GPU JPEG): {nv_jpg_time:.4f}s ({iterations/nv_jpg_time:.2f} it/s) -> {current_time/nv_jpg_time:.2f}x speedup")

    # --- 3. nvimagecodec Pipeline (TIFF) ---
    try:
        nvimagecodec_pipeline(tiff_img_path, coords[0][0], coords[0][1], crop_size, decoder)
        torch.cuda.synchronize()
        start_time = time.perf_counter()
        for x, y in coords:
        ...
        print(f"nvimagecodec   (GPU TIFF): {nv_tiff_time:.4f}s ({iterations/nv_tiff_time:.2f} it/s) -> {current_time/nv_tiff_time:.2f}x speedup")
    except Exception as e:
        print(f"nvimagecodec   (GPU TIFF): FAILED (Expected on Maxwell/Quadro M1200). Error: {e}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Benchmark nvimagecodec vs PIL")
    parser.add_argument("--data_dir", type=Path, default=Path("data/trompa-coco/trainval2017"), help="Directory containing images")
    parser.add_argument("--crop_size", type=int, default=3584, help="Size of the square crop")
    parser.add_argument("--iterations", type=int, default=50, help="Number of iterations to run")
    parser.add_argument("--image_name", type=str, default=None, help="Specific image file to use (e.g., image.png)")
