...
def filter_coco_json(anno_path: Path, pred_image_ids: set, out_path: Path):
    ...
    if out_path.exists():
        ...
        try:
            ...
            subset_img_ids = set(
                img["id"] for img in subset_data.get("images", [])
            )
            if subset_img_ids == pred_image_ids:
                print(
                    "Subset already matches requested images. Skipping heavy JSON load."
                )
                return
        except Exception:
    ...
    print(
        f"Loading full GT JSON from {anno_path} (this takes a moment but saves RAM later)..."
    )
    ...
    data["images"] = [
        img for img in data["images"] if img["id"] in pred_image_ids
    ]
    ...
    data["annotations"] = filtered_annotations[:100]
    print(
        f"Nuclear option: restricted to {len(data['annotations'])} ground truth annotations."
    )
...
def main(args):
    ...
    filtered_preds_path = (
        args.predictions_path.parent / "predictions_subset_eval.json"
    )
    ...
    dataset = fo.Dataset.from_dir(
        ...
        data_path=str(args.img_dir.absolute()),
    ...
    print(
        "Mapping COCO image IDs and converting TIFFs to PNG for browser compatibility..."
    )
    with open(subset_anno_path, "r") as f:
        subset_data = json.load(f)
    filename_to_id = {
        img["file_name"]: img["id"] for img in subset_data.get("images", [])
    }
    ...
    _, classes_map, _, _, _ = fouc.load_coco_detection_annotations(
        str(subset_anno_path)
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Visualize COCO GT and Predictions in FiftyOne"
    )
    parser.add_argument(
        "--img_dir",
        type=Path,
        default=Path(
            "/home/kwon-young/prog/music_deep/data/trompa-coco/trainval2017/"
        ),
