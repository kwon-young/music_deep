...
def random_patch_drop(
    ...
    kept_patches = torch.gather(
        patch_seq.patches, 1, indices.unsqueeze(-1).expand(-1, -1, patch_seq.patches.shape[-1])
    )
    kept_freqs = torch.gather(
        patch_seq.freqs, 1, indices.unsqueeze(-1).expand(-1, -1, patch_seq.freqs.shape[-1])
    )
