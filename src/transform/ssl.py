...
def random_crop[C: Channel, M: Mode, R: Range](
    sample: SSLSample[TensorImage[tuple[C, Height, Width], M, R]], crop_size: int
...
) -> SSLSample[FlatViewTensorImage[Literal[1], View, tuple[BatchView, *CHW], M, R]]:
...
def random_flatview_affine[B: Batch, V: View, M: Mode, R: Range](
    ...
    return SSLSample(image=_random_flatview_affine_img(sample.image, max_angle_deg, max_translate))
...
def extract_flatviewpatches[B: Batch, V: View, M: Mode, R: Range](
    ...
    return SSLSample(image=_extract_flatviewpatches_img(sample.image, patch_size))

@batched_transform
def random_flatview_patch_drop[B: Batch, BV: BatchView, V: View, N: NumPatches, D: EmbedDim | PatchDim](
    sample: SSLSample[FlatViewEmbeddings[B, BV, V, N, D]], drop_rate: float
) -> SSLSample[FlatViewEmbeddings[B, BV, V, NumPatches, D]]:
    return SSLSample(image=_random_flatview_patch_drop_img(sample.image, drop_rate))

def unflatten_views[B: Batch, BV: BatchView, V: View, N: NumPatches, D: EmbedDim | PatchDim](
...
def collate[Meta, V: View, C: Channel, H: Height, W: Width, M: Mode, R: Range](
    batch: tuple[Data[Meta, SSLSample[FlatViewTensorImage[Literal[1], V, tuple[BatchView, C, H, W], M, R]]], ...],
    batch_size: int,
) -> BatchedData[Meta, SSLSample[FlatViewTensorImage[Batch, V, tuple[BatchView, C, H, W], M, R]]]:
    ...
    return BatchedData(
        ...
        data=SSLSample(
            image=FlatViewTensorImage(
                stacked_data, num_views=num_views, original_batch_size=len(batch)
            )
        ),
    )
